...
class Accents:
    ...
    def load(self, data_dir, all_words):
        ...
        for fn in ['additional_rhymed_words.txt', 'rhymed_words.txt']:
...
# TODO - переделать детектирование неполных рифм так, как это сделано в коде рэпа.
fuzzy_ending_pairs0 = [
    #(r'\^эсна', r'\^эстна'), # интересно - честно
    (r'ж\^эе', r'ш\^эя'),  # word1=свежее word2=шея
    (r'\^очэм', r'\^очэнь'),  # впрочем - очень
    (r'\^энды', r'\^энты'),  # word1=легенды word2=ленты
    (r'\^атна', r'\^атный'),  # word1=пятна word2=невнятный
    (r'\^арэват', r'\^арива'),  # word1=пожаре вот word2=зарева
    (r'\^едитса', r'\^эдыца'),  # word1=съедется word2=медведица
    (r'\^ающэй', r'\^яющый'),  # word1=заступающей word2=сияющий
    (r'\^ызну', r'\^ысну'),  # word1=отчизну word2=повисну
    (r'([:C:])\^аю', r'[:1:]\^ая'),  # word1=прижимаю word2=немая
    (r'([:C:])\^эя', r'[:1:]\^эер'),  # word1=розовея word2=веер
    (r'\^этир', r'\^эчэр'),  # ветер - вечер
    (r'([:C:])\^оя', r'[:1:]\^ою'),  # word1=зноя word2=стеною
    (r'([:C:])\^этскым', r'[:1:]\^эцкых'),  # word1=детским word2=молодецких
    (r'чт\^а', r'чт\^а[:C:]+'),  # word1=мечта word2=почтамт
    (r'\^эртью', r'\^эрдью'),  # word1=смертью word2=твердью
    (r'\^антаф', r'\^анты'),  # word1=арестантов word2=педанты
    (r'\^оду', r'\^оту'),  # word1=народу word2=ноту
    (r'\^одцэ', r'\^оца'),  # word1=колодце word2=отзовётся
    (r'\^э([:C:])ья', r'\^э[:1:]ьэй'),  # word1=столетья word2=околеть
    (r'([:C:])\^эя', r'[:1:]\^эю'),  # xword1=хараш^эя xword2=ш^эю  word1=хорошея word2=шею
    (r'\^эзнаю', r'\^эзднаю'),  # word1=бесполезною word2=бездною
    (r'\^асным', r'\^астным'),  # word1=прекрасным word2=бесстрастным
    (r'\^астнам', r'\^асным'),  # word1=ненастном word2=красным
    (r'\^ящыйса', r'\^ащэйса'),  # word1=струящийся word2=молящейся
    (r'\^ымнэ', r'\^ымний'),  # word1=гимне word2=зимней
    (r'([:C:])\^ои', r'[:1:]\^ое'),  # word1=хвои word2=роковое
    (r'\^асные', r'\^азные'),  # word1=прекрасные word2=грязные
    (r'\^астные', r'\^асные'),  # word1=страстные word2=красные
    (r'\^утскам', r'\^уцкам'),  # word1=рекрутском word2=луцком
    (r'\^эцкай', r'\^этскай'),  # word1=немецкой word2=детской
    (r'\^ычьих', r'\^ычых'),  # word1=птичьих word2=лесничих
    (r'\^ызний', r'\^ызнэ'),  # word1=жизней word2=отчизне
    (r'\^оснай', r'\^осны'),  # word1=росной word2=сосны
    (r'\^эзднэ', r'\^эзны'),  # word1=бездне word2=болезни
    (r'\^осткы', r'\^оскы'),  # word1=подростки word2=боски
    (r'([:C:])\^эи', r'[:1:]\^эе'),  # word1=лилеи word2=аллее
    (r'\^оны', r'\^ёный'),  # word1=стоны word2=потаённый
    (r'\^ольны', r'\^ольний'),  # word1=колокольни word2=прикольней
    (r'\^азднык', r'\^азнык'),  # word1=праздник word2=проказник
    (r'\^отчэтний', r'\^отный'),  # word1=безотчетней word2=субботний
    (r'\^усамы', r'\^усымы'),  # word1=бусами word2=русыми
    (r'\^эшатьса', r'\^эшытьса'),  # word1=вешаться word2=тешиться
    (r'\^эрскае', r'\^эрская'),  # word1=дерзкое word2=офицерская
    (r'\^амыль', r'\^амэль'),  # нами ли - на мель
    (r'\^ятье', r'\^атья'),  # word1=объятье word2=проклятья
    (r'\^олвний', r'\^олный'),  # word1=безмолвней word2=молний
    (r'\^овнам', r'\^олвнам'),  # word1=условном word2=безмолвном
    (r'\^овний', r'\^овнэ'),  # word1=бескровней word2=часовне
    (r'\^ытвыны', r'\^ытвины'),  # word1=рытвины word2=молитвенны
    (r'\^озды', r'\^оздий'),  # word1=гвозди word2=гроздей
    (r'\^оргэ', r'\^оргый'),  # word1=восторге word2=георгий
    (r'\^ятьям', r'\^атьем'),  # word1=объятьям word2=заклятьем
    (r'дь\^ы', r'ть\^ы'),  # word1=ладьи word2=забытьи
    (r'ть\^ы', r'т\^ы'),  # word1=забытьи word2=пути
    (r'з\^а', r'с\^а'),  # word1=стезя word2=вся
    (r'\^ойный', r'\^ойна'),  # word1=заупокойный word2=спокойна
    (r'\^ёзны', r'\^озный'),  # word1=серьёзны word2=морозный
    (r'\^адастнай', r'\^агастный'),  # word1=радостной word2=благостный
    (r'\^эрныи', r'\^эрные'),  # word1=тернии word2=вечерние
    (r'\^астнае', r'\^астная'),  # word1=страстное word2=подвластная
    (r'\^ущамы', r'\^ущымы'),  # word1=кущами word2=цветущими
    (r'\^одная', r'\^однае'),  # word1=холодная word2=водное
    (r'\^агастнай', r'\^адастный'),  # word1=тягостной word2=радостный
    (r'\^яснай', r'\^асный'),  # word1=неясной word2=прекрасный
    (r'\^ытые', r'\^ытая'),  # word1=незабытые word2=залитая
    (r'\^ынымы', r'\^ынамы'),  # word1=синими word2=пустынями
    (r'\^атье', r'\^ятья'),  # word1=пожатье word2=объятья
    (r'\^аныи', r'\^аныя'),  # word1=лобзании word2=желания
    (r'\^эжыть', r'\^эшыть'),  # word1=нежить word2=утешить
    (r'\^окшэйса', r'\^огшэйса'),  # word1=запёкшейся word2=разлёгшейся
    (r'\^адчыца', r'\^ачэтса'),  # word1=отгадчица word2=прячется
    (r'\^озды', r'\^осты'),  # word1=гвозди word2=гости
    (r'\^ытилы', r'\^ытылы'),  # word1=родители word2=размагнитили
    (r'([:C:])\^ыа', r'[:1:]\^ые'),  # word1=колбасы word2=босые
    (r'\^олусы', r'\^оласам'),  # word1=гладиолусы word2=голосом
    (r'\^азднык', r'\^азны'),  # word1=праздник word2=казни
    (r'\^энитса', r'\^энытса'),  # word1=оденется word2=сменится
    (r'\^ужья', r'\^ушья'),  # ар^ужья xword2=равнад^ушья  word1=оружья word2=равнодушья
    (r'\^отствэ', r'\^отсвит'),  # word1=сходстве word2=отсвет
    (r'\^артыи', r'\^ардыи'),  # word1=партии word2=гвардии
    (r'\^астью', r'\^астьем'),  # word1=властью word2=счастьем
    (r'\^овы', r'\^овыи'),  # word1=крови word2=московии
    (r'\^эствые', r'\^эстыя'),  # word1=пришествие word2=известия
    (r'\^убылы', r'\^убила'),  # word1=пригубили word2=врубеля
    (r'[:A:]\^ы', r'[:A:]\^ы'),  # word1=толчеи word2=струи
    (r'\^асты', r'\^астье'),  # word1=отчасти word2=счастье
    (r'\^огае', r'\^огая'),  # word1=многое word2=трогая
    (r'\^адают', r'\^адуют'),  # word1=падают word2=радуют
    (r'\^янства', r'\^анствам'),  # word1=постоянство word2=пространством
    (r'\^ёнаю', r'\^оную'),  # word1=утаённою word2=сонную
    (r'\^осткэ', r'\^осткый'),  # word1=перекрёстке word2=жёсткий
    (r'\^эшна', r'\^эшный'),  # word1=утешно word2=грешный
    (r'\^олвлу', r'\^овлу'),  # word1=молвлю word2=готовлю
    (r'\^ыльний', r'\^ыльны'),  # word1=умильней word2=лесопильни
    (r'\^астнай', r'\^асный'),  # word1=властной word2=красный
    (r'([:C:])\^алысь', r'[:1:]\^алы'),  # word1=встречались word2=качали
    (r'([:C:])\^ая', r'[:1:]\^аю'),  # word1=разбирая word2=краю
    (r'ш\^ак', r'ш\^ах'),  # word1=кушак word2=ушах
    (r'\^аны', r'\^яный'),  # word1=туманны word2=постоянный
    (r'\^аснасты', r'\^аснастий'),  # word1=ясности word2=опасностей
    (r'\^атна', r'\^ятнай'),  # нивазвр^атна xword2=пры^ятнай  word1=невозвратно word2=приятной
    (r'\^ычыи', r'\^ычыя'),  # word1=безразличии word2=величия
    (r'\^эрчыва', r'\^эрчывай'),  # word1=недоверчиво word2=очерчивай
    (r'\^алинькый', r'\^алинкай'),  # word1=маленький word2=валенкой
    (r'\^эстые', r'\^эстыя'),  # word1=бесчестие word2=известия
    (r'\^ячыл', r'\^ачал'),  # word1=замаячил word2=начал
    (r'\^ятый', r'\^атай'),  # word1=объятый word2=лопатой
    (r'\^ящэй', r'\^ащэ'),  # word1=предстоящей word2=чаще
    (r'\^орна', r'\^орнай'),  # word1=упорно word2=чёрной
    (r'\^эшнай', r'\^эшна'),  # word1=грешной word2=неспешно
    (r'\^омны', r'\^ёмный'),  # word1=бездомны word2=неуёмный
    (r'\^ашэн', r'\^ажин'),  # word1=пашен word2=обезображен
    (r'\^эдны', r'\^этна'),  # word1=бедны word2=заметно
    (r'\^очэк', r'\^отчык'),  # word1=почек word2=лётчик
    (r'\^ышэн', r'\^ыжин'),  # word1=слышен word2=неподвижен
    (r'\^окрым', r'\^охры'),  # word1=мокрым word2=охры
    (r'\^эньем', r'\^энья'),  # word1=искушеньем word2=движенья
    (r'\^ошна', r'\^ожна'),  # раск^ошна xword2=м^ожна  word1=роскошна word2=можно
    (r'\^эньем', r'\^эньи'),  # word1=движеньем word2=самозабвеньи
    (r'\^ашкай', r'\^ашку'),  # word1=букашкой word2=рубашку
    (r'\^атьицэ', r'\^атытса'),  # word1=платьице word2=поплатится
    (r'\^ычка', r'\^ычкай'),  # word1=птичка word2=привычкой
    (r'([:C:])\^эет', r'[:1:]\^эи'),  # word1=холодеет word2=злодеи
    (r'\^участь', r'\^учась'),  # word1=участь word2=мучась
    (r'\^оскэ', r'\^осткэ'),  # word1=киоске word2=извёстке
    (r'\^олвным', r'\^олнам'),  # word1=безмолвным word2=волнам
    (r'\^альная', r'\^альнае'),  # word1=сальная word2=печальное
    (r'\^аныи', r'\^аные'),  # word1=издании word2=название
    (r'\^этская', r'\^эцкая'),  # word1=детская word2=немецкая
    (r'\^эстние', r'\^эснию'),  # word1=известнее word2=песнею
    (r'\^ыпку', r'\^ыпкай'),  # word1=ошибку word2=улыбкой
    (r'\^ые', r'\^ыи'),  # word1=родные word2=россии
    (r'\^алыя', r'\^алие'),  # word1=италия word2=далее
    (r'\^ятья', r'\^атью'),  # word1=объятья word2=кроватью
    (r'\^ютнай', r'\^утный'),  # word1=уютной word2=минутный
    (r'\^астным', r'\^аснам'),  # word1=несчастным word2=ужасном
    (r'\^однаю', r'\^однае'),  # word1=благородною word2=природное
    (r'\^яным', r'\^анам'),  # word1=пьяным word2=туманом
    (r'\^адыны', r'\^адины'),  # word1=гадины word2=украдены
    (r'\^эсные', r'\^эстные'),  # xword1=ниб^эсные xword2=бизв^эстные  word1=небесные word2=безвестные
    (r'\^аздная', r'\^азная'),  # word1=праздная word2=безобразная
    (r'\^ожамы', r'\^ожымы'),  # word1=вельможами word2=пригожими
    (r'\^адыш', r'\^атыш'),  # word1=сладишь word2=платишь
    (r'\^эдушкы', r'\^этушкы'),  # word1=соседушки word2=детушки
    (r'\^ожыей', r'\^ожие'),  # word1=божией word2=пригожее
    (r'\^эжию', r'\^эшэю'),  # word1=невежею word2=депешею
    (r'\^одну', r'\^отну'),  # word1=преисподню word2=сотню
    (r'\^адуя', r'\^атуя'),  # word1=досадуя word2=статуя
    (r'\^осткая', r'\^оская'),  # word1=жёсткая word2=плоская
    (r'\^эриный', r'\^эраный'),  # word1=уверенный word2=потерянный
    (r'\^ожые', r'\^ожыи'),  # word1=прохожие word2=божии
    (r'\^ызаны', r'\^ысаны'),  # word1=нанизаны word2=написаны
    (r'\^оратам', r'\^орадам'),  # word1=воротом word2=городом
    (r'\^азна', r'\^аздна'),  # word1=однообразно word2=праздно
    (r'\^азднай', r'\^азный'),  # word1=праздной word2=однообразный
    (r'\^арку', r'\^аркай'),  # word1=сударку word2=сигаркой
    (r'\^ошна', r'\^ощна'),  # word1=роскошно word2=нощно
    ...
    (r'\^ы([:C:]+)ы', r'\^ы[:1:]ый'),  # xword1=скр^ыпкы xword2=з^ыпкый  word1=скрипки word2=зыбкий
    (r'\^у([:C:]+)ы', r'\^у[:1:]ый'),  # word1=шутки word2=жуткий
    (r'\^отнай', r'\^отна'),  # word1=плотной word2=бесповоротно
    (r'\^омнай', r'\^омна'),  # word1=тёмной word2=вероломно
    (r'([:C:])\^ую', r'[:1:]\^уа'),  # word1=небольшую word2=прошу
    (r'\^отный', r'\^одный'),  # word1=пустотный word2=превосходный
    (r'\^усть', r'\^уст'),  # word1=грусть word2=хруст
    (r'\^ае', r'\^аи'),  # word1=мае word2=стаи
    (r'\^альна', r'\^альным'),  # xword1=иди^альна xword2=ри^альным  word1=идеально word2=реальным
    (r'\^эльем', r'\^элью'),  # word1=весельем word2=новоселью
    (r'\^ушбы', r'\^ужбы'),  # word1=душ word2=дружбы
    (r'\^о([:C:])ась', r'\^о[:1:]ас'),  # xword1=бар^олась xword2=г^олас  word1=боролась word2=голос
    (r'\^ы([:C:])ыи', r'\^ы[:1:]ыей'),  # xword1=изаб^ылыи xword2=л^ылыей  word1=изобилии word2=лилией
    (r'\^а([:C:])ымы', r'\^а[:1:]амы'),  # word1=бранными word2=буранами
    (r'\^ажду', r'\^аждай'),  # xword1=ж^ажду xword2=к^аждай  word1=жажду word2=каждой
    (r'\^одам', r'\^отам'),  # word1=непогодам word2=потом
    (r'\^ушу', r'\^ужу'),  # word1=душу word2=наружу
    (r'[сз]г\^ы[:C:]', r'[сз]к\^ы'),  # word1=изгиб word2=носки
    (r'\^ацать', r'\^адцать'),  # word1=мацать word2=двенадцать
    (r'\^узы', r'\^усы'),  # word1=музы word2=вкусы
    (r'\^аснай', r'\^асна'),  # word1=прекрасной word2=ясно
    (r'\^озний', r'\^озны'),  # word1=нервозней word2=козни
    ...
    (r'\^этлы', r'\^этьлы'),  # xword1=п^этлы xword2=сп^этьлы  word1=петли word2=спеть
    (r'\^учины', r'\^учыны'),  # xword1=пар^учины xword2=изл^учыны  word1=поручены word2=излучины
    (r'\^анамы', r'\^янымы'),  # xword1=ристар^анамы xword2=пь^янымы  word1=ресторанами word2=пьяными
    (r'\^а', r'^\^а'),  # xword1=едв^а xword2=^а  word1=едва word2=а
    (r'\^усава', r'\^усывать'),  # xword1=бр^усава xword2=зак^усывать  word1=брюсова word2=закусывать
    (r'\^ысаю', r'\^ысые'),  # xword1=лар^ысаю xword2=л^ысые  word1=ларисою word2=лысые
    (r'\^ось', r'\^ос'),  # xword1=павил^ось xword2=^ос  word1=повелось word2=ос
    (r'\^эныкы', r'\^эныкам'),  # xword1=шызафр^эныкы xword2=в^эныкам  word1=шизофреники word2=веником
    ...
    (r'\^эщы', r'\^эзчэ'),  # word1=вещи word2=резче
    (r'\^астнае', r'\^ясная'),  # word1=бесстрастное word2=неясная
    (r'\^ящый', r'\^ащэй'),  # word1=таящий word2=томящей
    (r'([:C:])\^эй', r'[:1:]\^эе'),  # word1=тополей word2=аллее
    (r'([:C:])\^ы', r'[:1:]\^ыи'),  # word1=зари word2=марии
    (r'\^этствэ', r'\^этствый'),  # word1=детстве word2=соответствий
    (r'\^еный', r'\^эний'),  # word1=строений word2=теней
    (r'\^ы([:C:])я', r'\^ы[:1:]ья'),  # word1="один я" word2=унынья
    (r'([:C:])\^ую', r'[:1:]\^уя'),  # word1=небольшую word2="прошу я"
    (r'\^эзднай', r'\^эзнай'),  # word1=бездной word2=бесполезной
    (r'\^о([:C:])я', r'\^о[:1:]ья'),  # word1="знаком я" word2=комья
    (r'\^эзднай', r'\^эзна'),  # word1=бездной word2=бесполезно
    (r'\^атэ', r'\^ятый'),  # word1=гекате word2=объятий
    (r'\^уткымы', r'\^уткамы'),  # word1=чуткими word2=проститутками
    (r'\^ыныи', r'\^ыные'),  # word1=линии word2=синие
    (r'\^эсытса', r'\^эсаца'),  # word1=привесится word2=месяца
    (r'\^эстнай', r'\^эсна'),  # word1=неуместной word2=тесно
    (r'\^эфскай', r'\^ефскый'),  # word1=невской word2=достоевский
    (r'\^ашны', r'\^ашний'),  # word1=башни word2=бесстрашней
    (r'\^ожыны', r'\^ожины'), # word1=изложины word2=стреножены
    (r'\^эвнамы', r'\^эвнымы'),  # word1=царевнами word2=напевными
    (r'\^очэтса', r'\^очыца'),  # word1=хочется word2=пророчица
    (r'\^адна', r'\^адный'),  # word1=жадно word2=отрадный
    (r'\^эсать', r'\^эсыть'),  # word1=десять word2=перевесить
    (r'\^одыя', r'\^одые'),  # word1=мелодия word2=благородие
    (r'([:C:])\^аи', r'[:1:]\^ая'),  # word1=трамваи word2=оживая
    (r'\^янье', r'\^аньем'),  # word1=сиянье word2=грохотаньем
    (r'\^аютса', r'\^аетса'),  # word1=качаются word2=прощается
    (r'\^олитса', r'\^олытса'),  # word1=колется word2=молится
    ...
    (r'\^([:A:])([:C:]+)[ыэ]й', r'\^[:1:][:2:][ыэ]'),  # первый-нервы

    (r'([:C:]+\^[ыэ])[й]', r'([:1:]+\^[ыэ])'),  # свиней-войне

    (r'\^([:A:])([:C:]+)[оуа][мн]', r'\^[:1:][:2:][оау]'),  # сонетом - Света

    (r'([:C:])\^([:A:])н', r'[:1:]\^[:2:]'),  # Антон - манто

    (r'\^([:A:])([:C:]+)+[оуа]', r'\^[:1:][:2:][оау]'),  # ложа - кожу
...
STOP_YO = ['нем', 'лес', 'небо', 'небе', 'небу', 'небом', 'неба', 'все', 'осел', 'всем',
    ...
    for i, c in enumerate(lword):
        if c in VOWELS:
            ...
            if v_counter == stress_pos:
                ...
                if ud_tags is not None and ('ADJ' in ud_tags or 'DET' in ud_tags or 'VerbForm=Part' in ud_tags) and ending == 'ого':
                    # местного ==> мэстнава
                    phonems.extend('ава')
                    break
            ...
            if c == 'ь' and phonems and phonems[-1] == 'ч':
                # мягкий знак после "ч" убираем: плачьте
                continue
            else:
                phonems.append(c)
...
def rhymed_fuzzy2_base(accentuator, word1, stress1, xword1, clausula1, word2, stress2, xword2, clausula2):
    ...
    if len(clausula1) >= 3:
        ...
        # Клаузуллы достаточно длинные и отличаются только финальным "й':
        # украдкой - сладко
        if clausula1+'й' == clausula2 or clausula1 == clausula2+'й':
            return True

    if len(clausula1) >= 4 and len(clausula2) >= 4:
        # Клаузулы отличаются только финальным "ь":
        # хотелось - Делос
        if clausula1+'ь' == clausula2 or clausula1 == clausula2+'ь':
            return True
